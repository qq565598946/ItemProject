...
var versionCon = GetQueryString('version');   // 版本
...
$(function () {
    ...
    $("#solutionVersion").val(versionCon);  // 版本
...
// 实施方案详情
function GetitemDestail(idFlag) {
...
function solutionDetail(solutioninfoList) {
    ...
    for (var i = 0; i < solutioninfoList.length; i++) {
                                                    ...
                                                    <div class="layui-inline">
                                                        ...
                                                        <input type="tel" name="phone"  autocomplete="off" class="layui-input solutionVersion"  disabled value="` + solutioninfoList[i].solutionVersion + `" >
                                                            ...
                                                            <div class="layui-inline">
                                                                ...
                                                                <div class="layui-input-inline">
                                                                    <input type="tel" name="phone"  autocomplete="off"  disabled class="layui-input kfdevelopOrgan" id="kfdevelopOrgan" value="` + solutioninfoList[i].developOrgan + `">
                                                                </div>
                                                            </div>
                                                    <div class="layui-inline">
        ...
        if(staffVlaueCon.length>0){         // 判断有值的话 就渲染分配开发人员DOM
            ...
            $(".staff-box-edit-done").html(staffHTML);
    ...
    for (var i = 0; i < overviewList.length; i++) {
        var overviewHTML = `<div class="overview-box edit-back-box">
                                <div class="edit-con-box-btn">
                                    <div class="edit-btn" onclick="editOverview(this)" data-edit="` + overviewList[i].id + `"><a href="#pj-overview"><i class="iconfont icon-bianji"></i></a></div>
                              </div> 
                            <div class="secLevel">
...
function DetailDemanalysis(demanalysisList, demanalysisAuditList) {
    ...
    for (var i = 0; i < demanalysisList.length; i++) {
        var demanHtml = `  <div class="box-content-deman edit-back-box">
                                <div class="edit-con-box-btn">
                                    <div class="edit-btn"  onclick="editdemanna(this)" data-edit="` + demanalysisList[i].id + `"><a href="#pj-demandAnalysis"><i class="iconfont icon-bianji"></i></a></div>
                                 </div> 
                                <div class="secLevel">
                                    <div class="pj-item-body-top">
...
function DetailArchidesign(saveArchidesign, archidesignAuditList) {
    ...
    for (var i = 0; i < saveArchidesign.length; i++) {
        var designHTML = `<div class="new-add-destail edit-back-box">
                                     <div class="edit-con-box-btn">
                                    <div class="edit-btn" onclick="designEditdata(this)" data-edit="` + saveArchidesign[i].id + `"><a href="#pj-design"><i class="iconfont icon-bianji"></i></a></div>
                                 </div> 
                            <div class="secLevel">
                                <div class="pj-item-body-top">
...
function Detaildeployrun(risksolutionList, risksolutionAuditList) {
    ...
    for (var i = 0; i < risksolutionList.length; i++) {
        var deployHTMKL = `<div class="secLevel">
                          ...
                          `;
        $(".edit-solution-box").html(deployHTMKL);
    }
...
function showsolution() {
    $(".edit-solution-box").hover(function () {
        $(this).find(".editOprate").show();
    },function () {
        $(this).find(".editOprate").hide();
    })
}
...
function basicinfo() {
    ...
    savejson.developOrgan = $("#kfdevelopOrgan").val();         // 开发单位
...
function editbasicinfo(_this) {
    ...
    $("#kfdevelopOrgan").val($(".kfdevelopOrgan").val());
    $(".close-deploy").show();
...
function savestaff() {
    // var staffValue = $("#staffvalue_two").val();
...
function ServicesumbitData() {
    let businessLen  =  formSelects.value('business_select', 'valStr');
    let technicalLen =  formSelects.value('technical_select', 'valStr');
    if(businessLen.length<1||technicalLen.length<1){
        layer.msg("请选择依赖的服务！")
        return
    }
    var infoJson = {};
...
function pjservice(flag) {
    ...
    $(".service-add-box textarea").val("");
    $("#pj-service .levelTips").hide();
    $(".service-add-box").show();
    $("#serviceName").val("");
...
function addserviceTemplate(name,type,design,business,technical,deploymentEnvironment,runResource,_this) {
    ...
    $(".service-add-box").html(addHtml);
    var Businesslable = business.split(",");   // 获取依赖的基础服务-通用业务服务
    var technicallable = technical.split(","); // 获取依赖的基础服务-基础技术服务
    GetserviceData("business_select","technical_select",Businesslable,technicallable,3);  // 勾选已有数据
    $("#service-option  option[value = " + type + "]").attr("selected", "selected");
    $("#service  option[value = " + deploymentEnvironment + "]").attr("selected", "selected");
    form.render();
...
form.on('submit(save-sumbit-application)', function(data){
    if(flagnumber==0){
        ...
        var appPlanningLen = editor.txt.text();
        if(appPlanningLen.length<1){
            layer.msg("请输入应用UI/UE规划")
            return
        }
...
function applictionTemplate(applicationName,applicationOverview,appPlanningCon,applyLogicdiagram,frontIntegration) {
    ...
    var appHtml = `<div class="secLevel">
                    ...
                    <div class="secLevel">
                        ...
                        <div class="pj-item-body-bottom" id="applicationPlanning">
                             <div id="editor"></div>
                            <!--<textarea class="layui-textarea" lay-verify="applicationPlanning" placeholder="请输入应用UI/UE规划">`+applicationPlanning+`</textarea>-->
                        </div>
                    </div>
                    <div class="secLevel">
                        <div class="pj-item-body-top">
    ...
    $(".add-application-box").html(appHtml);
    editor.create();
...
function editapplication(_this) {
    ...
    var  frontIntegration   = formSelects.value('select-id'+idnumber+'', 'valStr');
...
$("#pj-solution .addItem-box").click(function () {
    ...
    $(".add-solution-box").show()
})

var solutionEditID = null; // 风险解决方案修改ID
function saveRisksolution() {
    // var riskSolution = $("#riskSolution").val().length;
    // if (riskSolution < 1) {
    //     layer.msg("请输入内容");
    //     return
    // }
    // var riskSolution = $("#riskSolution").val();
    ...
    xhsdk.ajax({
        ...
        success: function (res) {
            if (res.code == 200) {
                ...
                GetitemDestail(7);
                $(".add-box-tip .editCon-btn").attr("data-edit", res.data);
...
function editsolution(_this) {
    ...
    solutionEditID = edit;
    $(".edit-solution-box").hide();
    $(".add-solution-box").show();
    $(".add-solution-box .editCon-btn").hide();
    $("#pj-solution .sumbit-button-box").show();
    var editCont = $(".riskSolution").text();
    $("#riskSolution").val(editCont);
}
